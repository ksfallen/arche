...
public class EnumBuster<E extends Enum<E>> {
    ...
    public EnumBuster(Class<E> clazz, Class... switchUsers) {
        try {
            ...
            throw new IllegalArgumentException("Could not builder the class", e);
        }
    }
     ...
     * @param value           新增枚举名字
     * @param ordinal
     * @param additionalTypes 枚举构造方法参数
     * @param additional      枚举构造方法值
     * @return 枚举实例
     */
    public E make(String value, int ordinal, Class[] additionalTypes, Object[] additional) {
        try {
            ...
            throw new IllegalArgumentException("Could not builder enum", e);
        }
    }
    ...
    public static void main(String[] args) {
        EnumBuster<HumanState> buster = new EnumBuster<>(HumanState.class);
        HumanState ANGRY = buster.make("ANGRY", new Class[]{Integer.class, Integer.class}, new Object[]{3, 3});
        buster.addByValue(ANGRY);

        System.out.println(Arrays.toString(HumanState.values()));
        HumanState state = HumanState.values()[2];
