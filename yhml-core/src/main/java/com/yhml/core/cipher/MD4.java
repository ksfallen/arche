...
public class MD4 {
    ...
    public static String encode(String s) {
        return toHexString(mdfour(s.getBytes(Charsets.UTF_8)));
    }

    public static void main(String[] args) throws NoSuchAlgorithmException {
        TimeInterval it = new TimeInterval();
        it.start();
        ...
        byte[] result = md.digest(data.getBytes(Charsets.UTF_8));
        data = toHexString(result);
        System.out.println(data);

        for (int i = 1; i < count; i++) {
            md.reset();
            result = md.digest(data.getBytes(Charsets.UTF_8));
            data = toHexString(result);
        }

        System.out.println(it.intervalSecond()  + "s, code: " + data);
